...
- **Partial-recon Katana never crawled** ([recon/helpers/resource_enum/katana_helpers.py](recon/helpers/resource_enum/katana_helpers.py)) — Docker-in-Docker path mismatch: targets file written to recon container's `/tmp/`, but spawned katana's `-v /tmp:/tmp` resolved against the host daemon's `/tmp` (where the file didn't exist), so katana exited in ~1.5s with 0 URLs. Switched to `/tmp/anveshak/` (already host-shared, used by every other tool) and surface stderr on early exit.
...
- **Wiki** -- [anveshak.wiki/Chat-Skills.md](anveshak.wiki/Chat-Skills.md) catalog tables now list 46 skills across Active Directory / Tooling / Protocols / Technologies / Frameworks / API Security / Vulnerabilities / Cloud / Post-Exploitation; [anveshak.wiki/Global-Settings.md](anveshak.wiki/Global-Settings.md) "Import from Community" updated to "**46** shipped skills"; [anveshak.wiki/AI-Agent-Guide.md](anveshak.wiki/AI-Agent-Guide.md) `kali_shell` reference page expanded from 8 generic bullets to 13 enriched bullets covering all the new tooling
...
- **Wiki documentation** -- [anveshak.wiki/Global-Settings.md](anveshak.wiki/Global-Settings.md) gains a full **Tradecraft** section (resources screen + add-resource modal, all 51 Quick Add presets grouped by type, resource-type comparison) and new dedicated [anveshak.wiki/Tradecraft-Lookup.md](anveshak.wiki/Tradecraft-Lookup.md) tool page (verify vs session split, sequence diagrams, section picker, two-tier fetch, cache layer, SSRF guard, comparison vs `web_search` and FAISS KB)
...
- **Subdomain Takeover Detection module** (`recon/main_recon_modules/subdomain_takeover.py` + `recon/helpers/takeover_helpers.py` + `graph_db/mixins/recon/takeover_mixin.py`) -- three-engine layered scanner that finds dangling DNS records whose third-party target can still be claimed by an attacker (expired Heroku apps, decommissioned S3 buckets, dead GitHub Pages, orphaned NS delegations, etc.). Runs as a third parallel sibling in **GROUP 6 Phase A** alongside Nuclei and the GraphQL scanner -- all three consume shared inputs (`Domain`/`Subdomain`/`BaseURL`/alive URLs) and emit `Vulnerability` nodes with zero data dependency, so the Phase A fan-out becomes a 3-way `ThreadPoolExecutor(max_workers=3)` with `run_subdomain_takeover_isolated()` deep-copying `combined_result` to avoid dict races. Disabled by default via `SUBDOMAIN_TAKEOVER_ENABLED`. Key components:
  ...
  - **Shared work directory** -- runner allocates `tempfile.mkdtemp(prefix="anveshak_takeover_", dir="/tmp/anveshak")` (bind-mounted between recon container and host) so Docker-in-Docker sibling containers (nuclei, baddns) see the same paths. Directory is chmod 755 so the non-root baddns user inside the sidecar can read targets files; cleanup is guaranteed via `try/finally + shutil.rmtree(ignore_errors=True)`
...
- **Authentication system** -- Anveshak now requires login. Two roles are supported: `admin` (full control) and `standard` (restricted to own scope). Key features:
  - **Login page** -- styled login page with Anveshak branding, dark/light theme support, email + password authentication
  - **JWT sessions** -- signed tokens stored in httpOnly cookies with 7-day expiry. All routes are protected by Next.js middleware
  - **Admin account setup** -- `./anveshak.sh install`, `up`, `up dev`, and `update` automatically prompt for admin credentials in the terminal when no admin exists
  ...
  - **CLI password reset** -- `./anveshak.sh reset-password` to recover from a forgotten admin password
...
- **anveshak.sh update logic** -- agent container now always rebuilds (not just restarts) when any `agentic/` file changes, since source code is baked into the image without volume mount
...
- **Smart ingestion on install** -- `./anveshak.sh install` detects GPU and API key availability. On CPU without an API key, shows an interactive prompt with estimated times per source and lets the user choose quick start (~15 min) or full ingestion (~4 hours). With GPU or API key, ingests all sources automatically
...
- **Makefile for KB management** -- `knowledge_base/Makefile` with targets for build, update, rebuild, stats, and cleanup. All `anveshak.sh` KB commands use `MODE=docker` to run inside the agent container
...
- **Wiki documentation** -- new [Recon & Project Presets](https://github.com/samugit83/anveshak/wiki/Recon-Presets) wiki page with full guide, 21-preset reference table, and AI generation walkthrough. Updated Creating a Project (16 tabs), Project Settings Reference, Running Reconnaissance, Home, and Sidebar
...
- **Bottom bar visibility** -- PageBottomBar (node type filters, session controls, stats) now hidden for Reverse Shell, Anveshak Terminal, RoE, and JS Recon views. Only visible for Graph Map, Graph Views, and All Nodes
...
- **Amass Brute Force Wordlist Selector** -- configurable wordlist selection for Amass DNS brute forcing:
  ...
  - **jhaddix all.txt**: Jason Haddix's comprehensive subdomain wordlist (~2.18M entries compiled from certificate transparency, bug bounty findings, DNS datasets) baked into the `anveshak-recon` Docker image
...
- **AI-Assisted Development wiki page** — new contributor guide with two structured integration prompts (`ADD_AGENTIC_TOOL`, `ADD_RECON_TOOL`) and a 7-step iterative workflow for shipping zero-bug PRs using Claude Code. See [Wiki: AI-Assisted Development](https://github.com/samugit83/anveshak/wiki/AI-Assisted-Development).
...
- **"Remote Shells" renamed to "Reverse Shell"** — tab renamed for clarity to distinguish from the new Anveshak Terminal tab. The Reverse Shell tab manages agent-opened sessions (meterpreter, netcat, etc.), while Anveshak Terminal provides direct interactive sandbox access.
...
- **Animated Loading Indicator** — replaced static "Processing..." text in the AI assistant chat with a dynamic loading experience:
  - **Anveshak eye logo** with randomized heartbeat animation (2–6s random intervals)
...
- **Pentest Report Generation** — generate professional, client-ready penetration testing reports as self-contained HTML files from the `/reports` page. Reports compile all reconnaissance data, vulnerability findings, CVE intelligence, attack chain results, and remediation recommendations into an 11-section document (Cover, Executive Summary, Scope & Methodology, Risk Summary, Findings, Other Vulnerability Details, Attack Surface, CVE Intelligence, GitHub Secrets, Attack Chains, Recommendations, Appendix). Features include:
  ...
  - **Wiki documentation** — new [Pentest Reports](anveshak.wiki/20.-Pentest-Reports) wiki page with example report download
...
- **GVM Vulnerability Scanning** — full end-to-end integration of Greenbone Vulnerability Management (GVM/OpenVAS) into the Anveshak pipeline:
...
- **Frontend Usability** — unified Anveshak primary crimson color (#d32f2f), styled message containers with ghost icons and gradient backgrounds, improved markdown heading and list spacing
