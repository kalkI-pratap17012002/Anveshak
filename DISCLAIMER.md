...
Anveshak is an **educational and research tool** designed exclusively for:
...
Anveshak's AI agent operates **autonomously**, making real-time decisions about which tools to invoke, which vulnerabilities to exploit, and how to chain attack steps — with minimal or no human intervention. By using this tool in autonomous mode (i.e., with approval gates disabled), you acknowledge and accept that:
...
Anveshak integrates, bundles, or invokes the following third-party open-source tools. Each tool is governed by its own license and terms. **The authors of Anveshak do not own, maintain, or provide warranty for any of these tools.** Users must independently comply with each tool's license:
