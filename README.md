...
<p align="center">
  <a href="https://www.anveshak.org/"><img height="34" src="https://img.shields.io/badge/🌐_Website-anveshak.org-A01025?style=for-the-badge&labelColor=000000" alt="Anveshak Website"/></a>
  ...
  <a href="https://t.me/anveshak_ai"><img height="34" src="https://img.shields.io/badge/Telegram-Join_Channel-26A5E4?style=for-the-badge&logo=telegram&logoColor=white&labelColor=000000" alt="Telegram Channel"/></a>
</p>

<br/>

<p align="center">
  <a href="https://github.com/samugit83/anveshak/stargazers"><img height="24" src="https://img.shields.io/github/stars/samugit83/anveshak?style=flat&color=2E8B57&label=Stars" alt="GitHub Stars"/></a>
  ...
  <a href="https://github.com/samugit83/anveshak/wiki/Fireteam-Parallel-Specialists"><img height="24" src="https://img.shields.io/badge/%F0%9F%94%A5%20FIRETEAM-PARALLEL%20MULTI--AGENT-7C3AED?style=flat" alt="Fireteam Parallel Multi-Agent"/></a>
  ...
  <a href="https://github.com/samugit83/anveshak/wiki"><img height="24" src="https://img.shields.io/badge/📖_WIKI-FULL%20DOCUMENTATION-1A73E8?style=flat" alt="Wiki Documentation"/></a>
</p>
...
<p align="center">
  <img src="assets/agent.gif" alt="Anveshak Agent Demo" width="100%"/>
</p>
<p align="center">
...
<p align="center">
<img src="assets/anveshak-graph.gif" alt="Anveshak Neo4j attack-surface graph rendered in 2D" width="100%"/>
</p>
<p align="center">
...
<br/><br/>
Anveshak doesn't stop at finding vulnerabilities, it fixes them. The pipeline starts with a 6-phase reconnaissance engine that maps your target's entire attack surface, then hands control to an autonomous AI agent that validates CVE exploitability, tests credential policies, and maps lateral movement paths. Every finding is recorded in a Neo4j knowledge graph. When the offensive phase completes, CypherFix takes over: an AI triage agent correlates hundreds of findings, deduplicates them, and ranks them by exploitability. Then a CodeFix agent clones your repository, navigates the codebase with 11 code-aware tools, implements targeted fixes, and opens a GitHub pull request, ready for review and merge.
</p>
...
```bash
git clone https://github.com/samugit83/anveshak.git
cd anveshak
...
At the end of the install (and on every `./anveshak.sh up` or `./anveshak.sh update` if no admin exists), you will be prompted in the terminal:
...
All settings are stored per-user in the database. See the **[AI Model Providers](https://github.com/samugit83/anveshak/wiki/AI-Model-Providers)** wiki page for detailed setup instructions.

### 4. Open the Webapp

Go to **http://localhost:3000** -- create a project, configure your target, and start scanning.

> For a detailed walkthrough of every feature, check the **[Wiki](https://github.com/samugit83/anveshak/wiki)**.
>
> Having issues? See the **[Troubleshooting](readmes/TROUBLESHOOTING.md)** guide or the **[Wiki Troubleshooting](https://github.com/samugit83/anveshak/wiki/Troubleshooting)** page.
...
| Command | Description |
|---------|-------------|
| `./anveshak.sh install` | Build + start lightweight (no GVM, no Knowledge Base, Tavily-only web search) |
| `./anveshak.sh install --kbase` | Build + start with the local Knowledge Base (~4.4 GB heavier) |
| `./anveshak.sh install --gvm` | Build + start with GVM/OpenVAS |

> Flags can be combined: `./anveshak.sh install --gvm --kbase`

| Command | Description |
|---------|-------------|
| **`./anveshak.sh update`** | **Pull latest version, smart-rebuild only changed services (preserves your install-time GVM/KB choice)** |
| `./anveshak.sh up` | Start services (auto-detects GVM and KB mode from install) |
| `./anveshak.sh up dev` | Start in dev mode with hot-reload (auto-detects GVM and KB mode) |
| `./anveshak.sh down` | Stop services (preserves data) |
| `./anveshak.sh status` | Show running services, version, GVM mode, KB state |
| `./anveshak.sh clean` | Remove containers + images, keep data |
| `./anveshak.sh reset-password` | Reset a user's password from the terminal |
...
```bash
./anveshak.sh up dev           # auto-detects GVM mode from install
```
...
docker image prune -f                             # Remove dangling images (auto-run by `./anveshak.sh update`)
...
**Default behavior:** The KB is **opt-in**. `./anveshak.sh install` produces a lightweight install (~4.4 GB lighter, Tavily-only web search). To enable the local KB, pass `--kbase`:

```bash
./anveshak.sh install --kbase
```

On first install with `--kbase`, Anveshak detects your hardware (GPU / CPU / API) and offers a quick-start profile. The choice is persisted, so subsequent `update` / `up` commands respect it without re-passing the flag.
...
```bash
./anveshak.sh kb build lite          # Build with lite profile (~30-60s with API)
./anveshak.sh kb build standard      # Add NVD CVEs
./anveshak.sh kb update nvd          # Incremental NVD refresh
./anveshak.sh kb stats               # Show index statistics
./anveshak.sh kb rebuild lite        # Wipe and rebuild from scratch
```

> For full technical documentation -- query pipeline, data sources, ingestion profiles, scoring, security model -- see the **[Knowledge Base Technical Reference](readmes/README.KBASE.md)** or the **[Wiki: Knowledge Base & Web Search](https://github.com/samugit83/anveshak/wiki/Knowledge-Base-Web-Search)**.

---

## Table of Contents

- [Full Wiki Documentation](https://github.com/samugit83/anveshak/wiki)
- [Overview](#overview)
...
- [Anveshak HackLab](#anveshak-hacklab)
...
Anveshak is a modular, containerized penetration testing framework that chains automated reconnaissance, AI-driven exploitation, and graph-powered intelligence into a single, end-to-end offensive security pipeline. Every component runs inside Docker (no tools installed on your host) and communicates through well-defined APIs so each layer can evolve independently.
...
> **[Wiki: Running Reconnaissance](https://github.com/samugit83/anveshak/wiki/Running-Reconnaissance)** | **[Technical: README.RECON.md](readmes/README.RECON.md)**

<p align="center">
  <img src="assets/recon.gif" alt="Anveshak Reconnaissance Pipeline" width="100%"/>
</p>
...
> **[Wiki: Recon Pipeline Workflow -- Partial Recon](https://github.com/samugit83/anveshak/wiki/Recon-Pipeline-Workflow#partial-recon)**
...
> **[Wiki: AI in Pipeline](https://github.com/samugit83/anveshak/wiki/Recon-Pipeline-Workflow#ai-in-pipeline)**
...
> **[Wiki: GVM Vulnerability Scanning](https://github.com/samugit83/anveshak/wiki/GVM-Vulnerability-Scanning)** | **[Technical: README.GVM.md](readmes/README.GVM.md)**
...
> **[Wiki: AI Agent Guide](https://github.com/samugit83/anveshak/wiki/AI-Agent-Guide)** | **[Technical: README.AGENTIC_SYSTEM.md](readmes/README.AGENTIC_SYSTEM.md)**

<p align="center">
  <img src="assets/exploit.gif" alt="Anveshak Exploitation Demo" width="100%"/>
</p>
...
> **Full operator manual** (every form field, all 39 presets, the auth flow, the live discovery workflow, validation rules, troubleshooting, and the storage / security model): **[MCP Tool Plugins wiki page](https://github.com/samugit83/anveshak/wiki/MCP-Tool-Plugins)**.

### Agent Workspace: Per-Project Filesystem, Background Jobs, Auto-Offload

> **Watch the demo:** [Anveshak Agent Workspace: AI Runs 4 Parallel Pentests and Writes Its Own Report (YouTube)](https://youtu.be/dkgIk78T7Hw)
...
> **Full operator manual** (every drawer feature, all 29 workspace tools, the auto-offload policy map, the four protected folders, fireteam coordination patterns via `fs_diff vs_last_read`, and the path-validation safety model): **[Agent Workspace wiki page](https://github.com/samugit83/anveshak/wiki/Agent-Workspace)**.

### Fireteam: Parallel Specialist Sub-Agents

The agent's most powerful execution mode. When an objective decomposes into **independent investigation angles** (auth surface, route map, header policy; or 5 candidate CVEs to triage in parallel), the root agent fans out into N **specialist sub-agents** that work concurrently inside the same backend, each running its own multi-step ReAct loop with a focused mission. This is Anveshak's implementation of the **Scatter-Gather ReAct (SG-ReAct)** architectural pattern: a root agent that decides when to fan out, a bounded fireteam of specialists that work in parallel, and a fan-in step that merges their findings back into a single consolidated worldview.
...
> **[Wiki: Fireteam (Parallel Specialists)](https://github.com/samugit83/anveshak/wiki/Fireteam-Parallel-Specialists)** | **[Technical: README.AGENTIC_SYSTEM.md](readmes/README.AGENTIC_SYSTEM.md#fireteam--parallel-specialist-sub-agents)**
...
> **[Wiki: AI Model Providers](https://github.com/samugit83/anveshak/wiki/AI-Model-Providers)**
...
> **[Wiki: Attack Surface Graph](https://github.com/samugit83/anveshak/wiki/Attack-Surface-Graph)** | **[Technical: GRAPH.SCHEMA.md](readmes/GRAPH.SCHEMA.md)**
...
> **[Wiki: EvoGraph](https://github.com/samugit83/anveshak/wiki/EvoGraph-Attack-Chain-Evolution)** | **[Technical: README.AGENTIC_SYSTEM.md](readmes/README.AGENTIC_SYSTEM.md#evograph--evolutive-attack-chain-graph)**
...
> **[Wiki: AI Agent Guide](https://github.com/samugit83/anveshak/wiki/AI-Agent-Guide)**
...
> **[Wiki: Reverse Shells](https://github.com/samugit83/anveshak/wiki/Reverse-Shells)**
...
> **[Wiki: The Graph Dashboard](https://github.com/samugit83/anveshak/wiki/The-Graph-Dashboard#anveshak-terminal)**
...
> **[Wiki: GitHub Secret Hunting](https://github.com/samugit83/anveshak/wiki/GitHub-Secret-Hunting)**
...
> **[Wiki: Project Settings Reference](https://github.com/samugit83/anveshak/wiki/Project-Settings-Reference)**

<p align="center">
  <img src="assets/settings.gif" alt="Anveshak Project Settings" width="100%"/>
</p>
...
> **[Wiki: Rules of Engagement](https://github.com/samugit83/anveshak/wiki/Rules-of-Engagement)**
...
> **[Wiki: Insights Dashboard](https://github.com/samugit83/anveshak/wiki/Insights-Dashboard)**

<p align="center">
  <img src="assets/insights.gif" alt="Anveshak Insights Dashboard" width="100%"/>
</p>
...
> **[Wiki: Creating a Project](https://github.com/samugit83/anveshak/wiki/Creating-a-Project)**
...
> **[Wiki: Data Export & Import](https://github.com/samugit83/anveshak/wiki/Data-Export-and-Import)**
...
| Resource | Link |
|----------|------|
| **Full Wiki** (user guide) | **[github.com/samugit83/anveshak/wiki](https://github.com/samugit83/anveshak/wiki)** |
| AI-Assisted Development | **[Wiki: Ship Perfect PRs with AI](https://github.com/samugit83/anveshak/wiki/AI-Assisted-Development)** |
...
Anveshak is fully Dockerized and runs on any OS with Docker Compose v2+. For OS-specific fixes (Linux, Windows, macOS), see **[Troubleshooting Guide](readmes/TROUBLESHOOTING.md)** or the **[Wiki](https://github.com/samugit83/anveshak/wiki/Troubleshooting)**.
...
<td width="280" align="center">
  <a href="https://github.com/samugit83/anveshak/wiki/Anveshak-HackLab">
    <img src="assets/hacklab.png" alt="Anveshak HackLab" width="260"/>
  </a>
</td>
<td>
  <h3>Want to see Anveshak think like a real pentester?</h3>
  ...
  <a href="https://github.com/samugit83/anveshak/wiki/Anveshak-HackLab"><b>Explore the HackLab &rarr;</b></a>
  &nbsp;&nbsp;|&nbsp;&nbsp;
  <a href="https://github.com/samugit83/anveshak/wiki/Anveshak-HackLab#community-sessions"><b>Submit your own session &rarr;</b></a>
...
Videos, writeups, and real-world experiences from security professionals using Anveshak in the field. Want to be featured? See the [Content Creator](CONTRIBUTING.md#content-creator) track in CONTRIBUTING.md.

### Videos

| Title | Link |
|-------|------|
| Anveshak v2.2.0, Social Engineering Test: Payload Delivery to Shell Access | [Watch](https://youtu.be/kVjV9K_eks4) |
| AI Agent CVE Validation: Beyond Standard Tooling | [Watch](https://youtu.be/rypmP1SJon8) |
| Anveshak 2.0, From 0 to 1000 GitHub Stars in 10 Days: Multi-Agent Parallel Attacks | [Watch](https://youtu.be/afViJUit0xE) |
...
| Who | What | Link |
|-----|------|------|
| Nipun Dinudaya | Deployed Anveshak on a company website, identified a critical SQL injection vulnerability that could have caused significant data exposure | [Read on LinkedIn](https://www.linkedin.com/posts/nipun-dinudaya-6159b32bb_anveshak-cybersecurity-penetrationtesting-ugcPost-7431233870253166592-aLvb) |
| Venkata Bhargav CH S | Used Anveshak during an internship at Ascent e-Digit Solutions: hands-on reconnaissance, DNS analysis, and attack surface mapping | [Read on LinkedIn](https://www.linkedin.com/posts/venkata-bhargav-cybersecurity_cybersecurity-ethicalhacking-redteam-share-7434940660803182592-e9En) |

### Community Guides

| Who | What | Link |
|-----|------|------|
| MrGood | Mastering Redamon: A Comprehensive Guide to Installation on Kali Linux, addressing Kali-specific Docker challenges and security posture | [Read on Medium](https://cyberaccoon.medium.com/mastering-anveshak-a-comprehensive-guide-to-installation-on-kali-linux-ea544e6f5b9f) |
| Bogdan Caraman | How to Install Anveshak on Debian 13 (Trixie) with OpenRouter, step-by-step guide with Docker setup, static IP, and systemd automation | [Read on Blog](https://blog.bogdancaraman.com/install-anveshak-debian-13-openrouter/) |
...
> **LOCAL USE ONLY**: Anveshak is designed to run on a **local machine** and has **not** been hardened for server or cloud deployment. It lacks the security controls required for a production environment exposed to the internet (e.g. authentication hardening, rate limiting, TLS enforcement, input sanitization across all surfaces). **Do not deploy Anveshak on a public-facing server.** Running it outside a trusted local network is entirely at your own risk.

This project is released under the [MIT License](LICENSE).

Anveshak integrates several third-party tools under their own licenses (MIT, Apache-2.0, BSD, GPL-2.0/3.0, AGPL-3.0, LGPL, and the WPScan Public Source License). Source code for all AGPL-licensed components is available at their upstream repositories. The full inventory and license obligations are documented in [THIRD-PARTY-LICENSES.md](THIRD-PARTY-LICENSES.md).

> **Commercial use note**: The Kali sandbox image bundles **WPScan**, which is governed by the WPScan Public Source License. WPScan restricts commercial use (SaaS, paid product offerings, value-added services) without a separate license from the WPScan team. Pentesting engagements and personal use are permitted. If you intend to use Anveshak in a commercial product or service, review [THIRD-PARTY-LICENSES.md](THIRD-PARTY-LICENSES.md) before distribution.
