...
5. **Automated path** via `kali_shell`:
   ...
   ```
6. **Validation**: capture `id`, `hostname`, `cat /etc/hostname`, and a UUID written into `/tmp/anveshak-<random>.txt`. Re-read via the same gadget to prove process identity.
