...
- **Payload/handler intent** ("generate", "payload", "reverse shell", "msfvenom", "handler", "listener", "one-liner", "backdoor"): request `transition_phase` to exploitation immediately. Do NOT generate payloads or start listeners from informational. The handler MUST be `exploit/multi/handler` via `metasploit_console` (only MSF sessions appear in the Anveshak UI). msfvenom generation via `kali_shell` is fine.
...
REACT_SYSTEM_PROMPT = """You are Anveshak, an AI penetration testing assistant using the ReAct (Reasoning and Acting) framework.
