"""
Anveshak Path Traversal / LFI / RFI Prompts
...
- Shannon vuln-injection.txt: OWASP-aligned phase flow (oracle -> confirmation
  ...
  / source-trace / deliverable-CLI instructions are stripped -- Anveshak agents
...
MARKER = '../../../../tmp/anveshak_zipslip_proof.txt'
...
ti = tarfile.TarInfo('../../../../tmp/anveshak_tarslip_proof.txt')
...
```
?file=../../../../tmp/anveshak_zipslip_proof.txt
```
...
- Absolute-path entries: some extractors ignore leading `/`; some honour it.
  Add `/etc/anveshak_check` entries alongside the `../` form to cover both.
