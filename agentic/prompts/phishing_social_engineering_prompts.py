"""
Anveshak Phishing / Social Engineering Prompts
...
**Only Metasploit handlers create tracked sessions that appear in the Anveshak Remote Shells UI.**
**A plain netcat listener will catch the shell but Anveshak cannot detect, display, or interact with it.**
...
| No session appears in Anveshak UI | You used `nc`/`netcat`/`socat` instead of Metasploit `exploit/multi/handler`. Kill the netcat listener and set up a proper handler via `metasploit_console`. Only Metasploit sessions are tracked by Anveshak. |
