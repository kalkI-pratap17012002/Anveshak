...
description: Reference for BloodHound + bhgraph (Anveshak NetworkX path-finder) covering collection, ownership marking, path-to-DA queries, edge-walk recipes per BloodHound edge type, and saturation strategy.
---

# BloodHound Path-to-DA Playbook

Reference for the Anveshak-specific `bhgraph` workflow: BloodHound JSON collection -> NetworkX in-memory graph -> path-to-DA queries -> edge-walk execution -> re-saturation. Pull this in when you have ANY valid domain credential and want a focused path-finder reference, separate from the broader `/skill ad_kill_chain`.
...
| Graph load + state | `kali_shell bhgraph load` | Anveshak-specific NetworkX path-finder over BloodHound JSON. |
