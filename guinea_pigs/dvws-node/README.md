...
A public instance of this environment is available at **http://gpigs.devergolabs.com** for testing with Anveshak.

### Acceptable Use Policy

By accessing this server you agree to the following terms:

1. **Anveshak-only testing** -- This server is provided exclusively for testing with the [Anveshak](https://github.com/samugit83/anveshak) framework. Manual exploitation, third-party scanners, and automated tools other than Anveshak are not authorized.
...
7. **Responsible disclosure** -- If you discover a vulnerability in Anveshak itself (not in the intentionally vulnerable target), report it via [GitHub Issues](https://github.com/samugit83/anveshak/issues) or email. Do not exploit vulnerabilities in Anveshak's infrastructure.
...
The default port-80 server keeps serving the landing page + DVWS proxy as before -- that landing page IS the baseline Anveshak compares against.

### Hidden vhosts on port 443 (TLS, L4 / SNI trick)

The nginx server presents a multi-SAN self-signed cert (CN=`gpigs.devergolabs.com`, SANs covering all hidden vhost names). Anveshak's httpx scrapes the SAN list and feeds the discovered names back into the VHost & SNI module's candidate set on the next run.
...
| `k8s.gpigs.devergolabs.com` | Either L4 or L7 -- BUT returns DIFFERENT bodies depending on which lane reached it | **high** -- `host_header_bypass` | `if ($ssl_server_name = ...)` returns ~2 KB JSON when SNI matched, ~1 KB JSON when only the Host header matched. Anveshak flags the L7 vs L4 disagreement as a routing-bypass primitive |

### Expected Anveshak output
...
```bash
# In Anveshak project settings: enable VHost & SNI, leave defaults
...
> **Cert is self-signed.** Browsers will warn -- accept the risk for testing. Anveshak's curl probes use `-k` so they bypass cert validation.
...
> **NEW:** TCP 443 is required for the [VHost & SNI demo](#vhost--sni-demo-anveshak-module-test-fixture). Without it the SNI-routed hidden vhosts (`internal.*`, `k8s.*`) are unreachable and only the L7 detection path can be exercised.

### 2. Deploy (first time or any update)

```bash
# from folder /anveshak
