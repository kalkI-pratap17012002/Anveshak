#!/bin/bash
# Install Docker and deploy Anveshak HackLab target environment
set -e
...
# The cert SANs include 7 hidden vhost hostnames so Anveshak's httpx scrapes them
...
<title>Anveshak HackLab -- Research Target</title>
<style>
...
<h1>Anveshak HackLab</h1>
<p class="subtitle">Research Target Server -- gpigs.devergolabs.com</p>

<div class="warning-box">
  <strong>WARNING:</strong> This server is a dedicated research target for authorized security testing with <a href="https://github.com/samugit83/anveshak">Anveshak</a> only. All traffic is logged and monitored. By accessing any service on this server, you accept the Rules of Engagement below.
</div>

<div class="info-box">
  This server hosts multiple services as part of the <a href="https://github.com/samugit83/anveshak">Anveshak</a> HackLab research environment. The Anveshak AI agent is designed to autonomously discover and map the attack surface. No additional information about the target is provided here intentionally -- the agent must perform its own reconnaissance.
</div>

<h2>Rules of Engagement</h2>
<ol>
  <li><strong>Anveshak-only testing.</strong> This server is provided exclusively for testing with the <a href="https://github.com/samugit83/anveshak">Anveshak</a> framework. Manual exploitation, third-party scanners, and automated tools other than Anveshak are not authorized.</li>
  ...
  <li><strong>Responsible disclosure.</strong> If you discover a vulnerability in Anveshak itself (not in the target), report it via <a href="https://github.com/samugit83/anveshak/issues">GitHub Issues</a>.</li>
...
<p style="margin-top:0.5rem;">
  <span class="badge">1</span> Install <a href="https://github.com/samugit83/anveshak">Anveshak</a> &nbsp;
  ...
  <span class="badge">5</span> Record and <a href="https://github.com/samugit83/anveshak/wiki/Anveshak-HackLab#community-sessions">submit your session</a>
</p>

<div class="footer">
  <a href="https://github.com/samugit83/anveshak">Anveshak</a> &middot;
  <a href="https://github.com/samugit83/anveshak/wiki/Anveshak-HackLab">HackLab Wiki</a> &middot;
...
# This is the BASELINE response when Anveshak's VHost & SNI module probes the
...
# Internal-keyword "admin" -> Anveshak flags as MEDIUM severity
# ===========================================================================
server {
    listen 80;
...
# Internal-keyword "staging" -> Anveshak flags as MEDIUM severity
# ===========================================================================
server {
    listen 80;
...
# Internal-keyword "jenkins" -> Anveshak flags as MEDIUM severity
# ===========================================================================
server {
    listen 80;
...
# -> Anveshak flags as LOW severity (status mismatch is the trigger)
# ===========================================================================
server {
    listen 80;
...
# Same status as baseline + slight body size delta -> Anveshak flags as INFO severity
# ===========================================================================
server {
    listen 80;
    ...
    location / {
        default_type text/html;
        return 200 '<!DOCTYPE html><html><head><title>Anveshak HackLab - News</title></head>
<body><h1>Anveshak HackLab News</h1><p>Latest research updates from the Devergolabs security team.</p><ul><li>2026-04-22: New attack-skill library released</li><li>2026-04-15: Improved Cypher query generation</li><li>2026-04-08: VHost & SNI Enumeration module added</li></ul></body></html>';
    }
}
...
# baseline Anveshak compares against when probing the IP with random SNI.
# ===========================================================================
server {
    ...
    location / {
        ...
        return 200 'Anveshak HackLab -- TLS endpoint. See / on port 80 for terms.\n';
    }
}

# ===========================================================================
# Hidden VHost #6: internal.gpigs.devergolabs.com (port 443 / SNI-routed L4)
# Internal-keyword "internal" -> Anveshak flags as MEDIUM severity via L4 path
...
# Host header matched via fallthrough (L7). Anveshak flags this as
...
echo "=== DONE ==="
echo ""
echo "Anveshak HackLab deployed successfully."
