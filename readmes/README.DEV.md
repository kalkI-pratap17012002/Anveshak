# Anveshak — Developer Guide

**Everything you need to understand, develop, and extend Anveshak.**
...
Every technology used in Anveshak, organized by layer. Each entry links to its **official documentation** so you can learn how it works.

> For detailed role descriptions of each technology within Anveshak, see [TECH_STACK.md](TECH_STACK.md).
...
Anveshak is a fully Dockerized system with 15+ containers communicating over two internal networks.
...
|---------|--------|---------|
| **`anveshak`** | bridge (default) | All inter-service communication |
...
|---------|---------------|------|------|
| `webapp` | anveshak-webapp | 3000 | Next.js frontend + backend API |
| `agent` | anveshak-agent | 8090 (→8080 internal) | AI agent (LangGraph + FastAPI WebSocket) |
| `recon-orchestrator` | anveshak-recon-orchestrator | 8010 | Spawns recon/GVM/GitHub containers via Docker SDK |
| `kali-sandbox` | anveshak-kali | 8000, 8002–8004 | MCP tool servers (nmap, nuclei, metasploit, network-recon) |
| `postgres` | anveshak-postgres | 5432 | PostgreSQL database |
| `neo4j` | anveshak-neo4j | 7474, 7687 | Neo4j graph database |
| `gvmd` | anveshak-gvm-gvmd | internal | GVM daemon (vulnerability scanner) |
| `gvm-ospd` | anveshak-gvm-ospd | internal | OpenVAS scanner engine |
...
| PostgreSQL | `postgresql://anveshak:anveshak_secret@postgres:5432/anveshak` |
...
Anveshak has **190+ project settings** that control everything from Katana crawl depth to Metasploit payload configuration.
...
> **Not a developer?** Use `./anveshak.sh install` for a single-command production setup. See the [README Quick Start](../README.md#quick-start).
...
docker compose exec postgres psql -U anveshak    # PostgreSQL interactive shell
...
docker compose exec postgres psql -U anveshak -c "SELECT * FROM \"Project\" LIMIT 5;"
...
AI-assisted development is welcome and encouraged. Anveshak is a large, multi-language, multi-container codebase — AI coding tools can significantly speed up development and help you navigate unfamiliar subsystems.
...
3. **Respect the architecture** — Anveshak has clear boundaries between subsystems (webapp, agent, recon, MCP). Don't let AI tools blur these boundaries by generating cross-cutting shortcuts that bypass the established communication patterns (REST, WebSocket, MCP protocol).
...
5. Backfill existing database rows if needed:
   ```bash
   docker compose exec postgres psql -U anveshak -c \
...
|----------|---------|-------------|
| `POSTGRES_USER` | `anveshak` | PostgreSQL username |
| `POSTGRES_PASSWORD` | `anveshak_secret` | PostgreSQL password |
| `POSTGRES_DB` | `anveshak` | PostgreSQL database name |
...
> The project also maintains a **[GitHub Wiki](https://github.com/samugit83/anveshak/wiki)** with additional guides and walkthroughs.
