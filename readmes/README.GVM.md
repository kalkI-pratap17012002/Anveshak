...
### Anveshak Integration

Anveshak uses GVM in **headless API mode** (no web GUI) to:
...
**Architecture:** The scan flow mirrors the recon pipeline: Webapp API → Recon Orchestrator → Docker container (`anveshak-vuln-scanner`) → SSE log streaming → graph update.
...
#### 1. **anveshak-gvm-gvmd** (Greenbone Vulnerability Manager Daemon)
...
#### 2. **anveshak-gvm-ospd** (OSPd-OpenVAS Scanner Daemon)
...
#### 3. **anveshak-gvm-postgres** (PostgreSQL Database)
...
#### 4. **anveshak-gvm-redis** (Redis Cache)
...
#### 5. **anveshak-gvm-notus-scanner** (Notus Scanner)
...
Anveshak uses **data containers** that download feeds once and populate Docker volumes:
...
│   ├── Dockerfile                    # Python scanner image (anveshak-vuln-scanner)
