...
```mermaid
flowchart TD
    CLI["CLI / Makefile / anveshak.sh<br/>python -m knowledge_base.curation.data_ingestion"]
...
graph TD
    subgraph "Docker Compose Stack"
        ...
        subgraph "Always Running"
            AGENT["agent<br/>anveshak-agent<br/>Port 8090:8080"]
            NEO4J["neo4j<br/>anveshak-neo4j<br/>Port 7474, 7687"]
        end

        subgraph "Opt-in (profile: kb-refresh)"
            REFRESH["kb-refresh<br/>anveshak-kb-refresh<br/>Shell sleep-loop scheduler"]
        end
...
- **Image**: `anveshak-agent` (built from `agentic/Dockerfile`)
...
- **Image**: Same `anveshak-agent` image, different entrypoint
- **Schedule**:
...
### anveshak.sh Commands

```bash
./anveshak.sh kb build [lite|standard|full]     # build with profile
./anveshak.sh kb update [source|all]            # incremental update
./anveshak.sh kb rebuild [lite|standard|full]   # wipe + rebuild
./anveshak.sh kb stats                          # show statistics
```

The `kb build` command is automatically triggered during `./anveshak.sh install`, `up`, and `restart` with the `lite` profile. Build failure is non-fatal -- the agent starts without KB.
...
During `./anveshak.sh install` (and `up`/`restart`), Anveshak automatically detects your capabilities:
...
```mermaid
flowchart TD
    subgraph "Entry Point 1: anveshak.sh install / up / restart"
        CMD1["./anveshak.sh install<br/>./anveshak.sh up<br/>./anveshak.sh restart"]
        ...
        EXEC1["docker exec anveshak-agent<br/>python -m knowledge_base<br/>.curation.data_ingestion<br/>--profile lite"]
    ...
    subgraph "Entry Point 2: Manual CLI"
        ...
        CMD2B["./anveshak.sh kb build standard<br/>./anveshak.sh kb update nvd"]

        CMD2A --> EXEC2["docker exec anveshak-agent<br/>python -m knowledge_base<br/>.curation.data_ingestion<br/>--profile/--source args"]
        CMD2B --> CMD2A
    end
    ...
    subgraph "Entry Point 4: Direct Python (local dev)"
        ...
        VENV["Requires: .anveshak-venv or<br/>active virtualenv with deps"]
        PY --- VENV
    end
...
|---|---|---|---|
| User runs `./anveshak.sh install` | Starts containers, then calls `_kb_bootstrap lite` | Host shell | `anveshak.sh:364-374` |
...
```bash
docker exec -it anveshak-agent python -c "
import asyncio
