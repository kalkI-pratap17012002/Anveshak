...
PostgreSQL database for storing Anveshak project configurations and user data. Uses Prisma ORM for schema management and query building.
...
# 2. Verify connection
docker exec anveshak-postgres pg_isready -U anveshak -d anveshak
...
services:
  postgres:
    ...
    container_name: anveshak-postgres
    environment:
      POSTGRES_USER: ${POSTGRES_USER:-anveshak}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-anveshak_secret}
      POSTGRES_DB: ${POSTGRES_DB:-anveshak}
    ports:
      - "5432:5432"
    volumes:
    ...
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U anveshak -d anveshak"]
      interval: 10s
      timeout: 5s
      retries: 5
...
```bash
POSTGRES_USER=anveshak
POSTGRES_PASSWORD=your_secure_password
POSTGRES_DB=anveshak
```
...
```bash
DATABASE_URL="postgresql://anveshak:anveshak_secret@localhost:5432/anveshak?schema=public"
```
...
```bash
# Create backup
docker exec anveshak-postgres pg_dump -U anveshak anveshak > backup.sql

# Create compressed backup
docker exec anveshak-postgres pg_dump -U anveshak anveshak | gzip > backup.sql.gz
```
...
```bash
# Restore from backup
docker exec -i anveshak-postgres psql -U anveshak anveshak < backup.sql

# Restore from compressed backup
gunzip -c backup.sql.gz | docker exec -i anveshak-postgres psql -U anveshak anveshak
```
...
docker exec -it anveshak-postgres psql -U anveshak -d anveshak
...
# Check database size
SELECT pg_size_pretty(pg_database_size('anveshak'));
```
...
# webapp/.env.local
DATABASE_URL="postgresql://anveshak:your_password@localhost:5432/anveshak"
```
