...
docker network create anveshak-network
...
| `RECON_IMAGE` | `anveshak-recon:latest` | Docker image for recon |
...
services:
  recon-orchestrator:
    build: .
    container_name: anveshak-recon-orchestrator
    ports:
      - "8010:8010"
    volumes:
    ...
    environment:
      ...
      - RECON_IMAGE=anveshak-recon:latest

networks:
  default:
    name: anveshak-network
    external: true
```
...
1. Check Docker socket is accessible:
   ```bash
   docker exec anveshak-recon-orchestrator docker ps
   ```

2. Verify recon image exists:
   ```bash
   docker images | grep anveshak-recon
   ```

3. Check orchestrator logs:
   ```bash
   docker logs anveshak-recon-orchestrator
   ```
