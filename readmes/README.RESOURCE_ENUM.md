# Anveshak - Resource Enumeration Module
...
| (Binary auto-download) | - | `~/.anveshak/tools/kiterunner/kr` | Auto-downloaded from GitHub releases |
...
2. **Download**: Fetches JS files to a temporary directory (`/tmp/anveshak/jsluice_<pid>/`)
...
3b. JSLUICE ANALYSIS (sequential, after parallel discovery)
   ...
   └── Download JS files to /tmp/anveshak/jsluice_<pid>/
...
```json
{
  "resource_enum": {
    "scan_metadata": {
      ...
      "kiterunner_enabled": true,
      "kiterunner_binary_path": "~/.anveshak/tools/kiterunner/kr",
      "kiterunner_wordlist": "apiroutes-251227",
...
Run Kiterunner manually (binary auto-downloads to ~/.anveshak/tools/kiterunner/):

```bash
# Binary location after first run
# Use -A flag for auto-downloaded wordlists
~/.anveshak/tools/kiterunner/kr scan https://example.com \
...
| Kiterunner binary | Auto-downloaded from GitHub releases to `~/.anveshak/tools/kiterunner/` |
...
*Documentation generated for Anveshak v1.0 - Resource Enumeration Module*
