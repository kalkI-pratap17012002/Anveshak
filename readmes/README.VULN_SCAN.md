# Anveshak - Nuclei Vulnerability Scanner
...
The `nuclei_scan.py` module integrates ProjectDiscovery's Nuclei scanner into Anveshak's reconnaissance pipeline. Nuclei is a fast, template-based vulnerability scanner that excels at web application security testing.
...
Anveshak automatically updates templates when `NUCLEI_AUTO_UPDATE_TEMPLATES = True` (default):
...
In addition to Nuclei templates, Anveshak includes custom Python-based security checks that detect vulnerabilities **not covered by Nuclei**.
...
*Documentation generated for Anveshak v1.0 - Nuclei Scanner Module with Katana DAST Integration*
