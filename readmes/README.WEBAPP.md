...
docker build -t anveshak-webapp:latest .

# Run production container locally
docker run -p 3000:3000 \
  --network graph_db_default \
  -e NEO4J_URI=bolt://anveshak-neo4j:7687 \
  -e NEO4J_USER=neo4j \
  -e NEO4J_PASSWORD=your_password \
  anveshak-webapp:latest
```
...
# Tag image
docker tag anveshak-webapp:latest \
  YOUR_ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/anveshak-webapp:latest
...
```json
{
  ...
  "containerDefinitions": [
    {
      "name": "webapp",
      "image": "YOUR_ACCOUNT.dkr.ecr.us-east-1.amazonaws.com/anveshak-webapp:latest",
      "portMappings": [
        {
          "containerPort": 3000,
          "protocol": "tcp"
      ...
      "secrets": [
        {
          "name": "NEO4J_PASSWORD",
          "valueFrom": "arn:aws:secretsmanager:us-east-1:YOUR_ACCOUNT:secret:anveshak/neo4j"
        }
      ],
      "healthCheck": {
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "/ecs/anveshak-webapp",
...
aws ecs create-service \
  ...
  --service-name anveshak-webapp \
  --task-definition anveshak-webapp \
  --desired-count 2 \
  --launch-type FARGATE \
...
aws application-autoscaling register-scalable-target \
  ...
  --resource-id service/your-cluster/anveshak-webapp \
  --min-capacity 2 \
  --max-capacity 100
...
|----------|-------------|---------|
| `NEO4J_URI` | Neo4j connection URI | `bolt://localhost:7687` (dev) / `bolt://anveshak-neo4j:7687` (Docker) |
| `NEO4J_USER` | Neo4j username | `neo4j` |
| `NEO4J_PASSWORD` | Neo4j password | (set in .env.local) |
| `DATABASE_URL` | PostgreSQL connection string | `postgresql://anveshak:anveshak_secret@localhost:5432/anveshak` |
...
The `AIAssistantDrawer` is the primary interface for interacting with the Anveshak pentesting agent. It communicates over WebSocket via the `useAgentWebSocket` hook and renders a real-time timeline of agent activity.
