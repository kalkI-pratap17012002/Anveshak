...
> **Full troubleshooting guide**: [Wiki — Troubleshooting](https://github.com/samugit83/anveshak/wiki/Troubleshooting)

## Operating System Compatibility

Anveshak is fully Dockerized and runs on **any OS** that supports Docker and Docker Compose v2+. Below are common OS-specific issues and their fixes.
...
| Volume mount fails | Windows path format incompatible with Linux containers | Run from inside WSL2 filesystem (`~/anveshak`), **not** from `/mnt/c/` |
...
| Containers OOM-killed during install | Docker Desktop default Memory below Anveshak minimum | Docker Desktop → Settings → Resources → Memory: set to 4 GB (8 GB with `--gvm`), then `./anveshak.sh install` |
| `Cannot connect to the Docker daemon` | Docker Desktop not started | `open /Applications/Docker.app` and wait until the whale icon stops animating before running `./anveshak.sh` |
...
| SYN scans (naabu, masscan) miss hosts on the local LAN | `network_mode: host` joins Docker Desktop's LinuxKit VM, not the Mac's network | Expected on Docker Desktop. Internet targets work normally; for LAN scans, run Anveshak on a Linux host |
| Login fails after install with no error in logs | Non-interactive shell (CI, agents, some multiplexers) injected a `\u0001` SOH byte into the admin email/password prompt | Run `./anveshak.sh reset-password` to recreate credentials cleanly |
