...
- **Container-based execution.** Partial recon runs inside the same `anveshak-recon` Docker image as the full pipeline, with a different command (`python /app/recon/partial_recon.py`). The orchestrator manages the container lifecycle.
...
User clicks Play on tool node (ProjectForm) 
  ...
  -> Orchestrator writes config JSON to /tmp/anveshak/, spawns recon container
...
6. Container manager: json.dump(config) -> /tmp/anveshak/partial_{project_id}.json
