...
the JSON output into Anveshak's Vulnerability dict shape.
...
# --------------------------------------------------------------------------
# Internal key (-e exclusion flag) -> Anveshak vulnerability_type.
...
def _normalize_findings(endpoint: str, raw_findings: List[dict]) -> List[dict]:
    """Convert graphql-cop JSON -> Anveshak Vulnerability dict shape.
...
def _map_severity(raw: str) -> str:
    """graphql-cop severity -> Anveshak canonical lowercase."""
    return {
        'HIGH':   'high',
        'MEDIUM': 'medium',
        'LOW':    'low',
