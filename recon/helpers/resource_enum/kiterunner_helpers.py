"""
Anveshak - Kiterunner API Discovery Helpers
==========================================
...
def _create_temp_dir(prefix: str = "kr") -> Path:
    """Create a temp directory under /tmp/anveshak for Docker-in-Docker compatibility."""
    temp_dir = Path(f"/tmp/anveshak/.{prefix}_{uuid.uuid4().hex[:8]}")
...
def ensure_kiterunner_binary(wordlist_name: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    """
    # Determine paths
    tools_dir = Path.home() / ".anveshak" / "tools"
    ...
    if not binary_path.exists():
        ...
        try:
            ...
            request = urllib.request.Request(
                download_url,
                headers={'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) Anveshak/1.0'}
            )
...
def run_kiterunner_discovery(
    ...
    # Create temp directory for targets file (use /tmp/anveshak for Docker-in-Docker compatibility)
...
def detect_kiterunner_methods(
    ...
    # Use /tmp/anveshak for Docker-in-Docker compatibility (avoids paths with spaces)
