...
def run_amass(domain: str, settings: dict = None) -> set:
    ...
    amass_temp = Path("/tmp/anveshak/.amass_temp")
...
def verify_domain_ownership(domain: str, token: str, txt_prefix: str = "_anveshak-verify") -> dict:
    """
    Verify domain ownership via DNS TXT record.

    Checks for a TXT record at {txt_prefix}.{domain} containing "anveshak-verify={token}".

    Args:
        domain: Root domain to verify (e.g., "example.com")
        token: Expected ownership token
        txt_prefix: DNS record prefix (default: "_anveshak-verify")
    ...
    expected_value = f"anveshak-verify={token}"
...
def run_puredns_resolve(subdomains: list, domain: str, settings: dict = None) -> list:
    ...
    # Prepare temp files in /tmp/anveshak (same path inside and outside container)
