"""
Anveshak - HTTP Probing Module + Banner Grabbing
...
def get_host_path(container_path: str) -> str:
    ...
    /tmp/anveshak is mounted to the same path inside and outside, so no translation needed.
    """
    # /tmp/anveshak paths are the same inside and outside the container
    if container_path.startswith("/tmp/anveshak"):
...
def run_http_probe(recon_data: dict, output_file: Path = None, settings: dict = None) -> dict:
    ...
    # Use /tmp/anveshak to avoid spaces in paths (snap Docker issue)
    scan_temp_dir = Path("/tmp/anveshak/.httpx_temp")
