"""
Anveshak - Subdomain Takeover Scanner Module
===========================================
...
def run_subdomain_takeover(
    ...
    """
    print("\n" + "=" * 70)
    print("[*][Takeover] Anveshak - Subdomain Takeover Scanner")
    print("=" * 70)
    ...
    # --------------------------------------------------------------
    started = time.time()
    # Use /tmp/anveshak (bind-mounted between recon container and host) so that
    # Docker-in-Docker sibling containers (nuclei, baddns) see the same paths.
    shared_root = Path("/tmp/anveshak")
    shared_root.mkdir(parents=True, exist_ok=True)
    work_dir = Path(tempfile.mkdtemp(prefix="anveshak_takeover_", dir=str(shared_root)))
...
def _run_baddns(
    ...
    Uses Docker-in-Docker against `anveshak-baddns:latest` (built separately
    ...
    AGPL isolation: Anveshak never imports baddns; the process + filesystem
    ...
    baddns_image = settings.get("BADDNS_DOCKER_IMAGE", "anveshak-baddns:latest")
    ...
    # Convert container path → host path for Docker-in-Docker. /tmp/anveshak
    ...
    container_name = f"anveshak-baddns-{os.getpid()}-{int(time.time())}"
...
def _probe_for_ai_disambiguation(hostname: str, timeout: int = 10):
    ...
    schemes = ("https", "http")
    headers_base = {"User-Agent": "Mozilla/5.0 (compatible; Anveshak/4.7)"}
