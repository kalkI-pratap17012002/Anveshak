...
Fix: write targets to /tmp/anveshak (same path host<->recon container per
docker-compose.yml) and mount /tmp/anveshak:/tmp/anveshak into katana.
...
class TestKatanaDinDPaths(unittest.TestCase):
    ...
    def test_url_file_written_under_tmp_anveshak(self):
        """Targets file must live under /tmp/anveshak (host-shared), not /tmp."""
        captured, _, _ = self._run_with_fake_subprocess()
        self.assertIsNotNone(captured["url_file"], "url_file was never opened")
        self.assertTrue(
            captured["url_file"].startswith("/tmp/anveshak/"),
            f"url_file must be under /tmp/anveshak/ for DinD; got {captured['url_file']}",
        )
        self.assertNotEqual(
    ...
    def test_docker_volume_mount_is_tmp_anveshak(self):
        """Spawned katana container must bind-mount /tmp/anveshak, not /tmp."""
        captured, _, _ = self._run_with_fake_subprocess()
        cmd = captured["cmd"]
        ...
        self.assertIn(
            "/tmp/anveshak:/tmp/anveshak", mounts,
            f"katana must mount /tmp/anveshak:/tmp/anveshak (DinD shared path); got mounts: {mounts}",
        )
        self.assertNotIn(
            "/tmp:/tmp", mounts,
    ...
    def test_list_arg_points_at_shared_path(self):
        ...
        targets file was written to, and that path must be under /tmp/anveshak."""
        captured, _, _ = self._run_with_fake_subprocess()
        cmd = captured["cmd"]
        ...
        self.assertTrue(
            list_path.startswith("/tmp/anveshak/"),
            f"-list path must be under /tmp/anveshak/; got {list_path}",
        )
