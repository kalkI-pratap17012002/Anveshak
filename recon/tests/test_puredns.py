...
# Use a test-specific temp dir to avoid touching /tmp/anveshak or /app/recon
...
def test_successful_filtering():
    ...
    # The function writes to /tmp/anveshak and reads from /app/recon/data
    # We'll create real files and mock only subprocess.run
    data_dir = Path("/tmp/anveshak")
...
def test_command_construction_all_flags():
    ...
    resolver_shared = Path("/tmp/anveshak/resolvers.txt")
    ...
    # Cleanup
    for f in Path("/tmp/anveshak").glob("puredns_*"):
...
def test_input_file_content():
    ...
    def capture_run(cmd, **kwargs):
        ...
        input_path = Path("/tmp/anveshak/puredns_input_example.com.txt")
...
if __name__ == "__main__":
    # Ensure /tmp/anveshak exists with resolvers for filesystem-based tests
    Path("/tmp/anveshak").mkdir(parents=True, exist_ok=True)
    resolver = Path("/tmp/anveshak/resolvers.txt")
