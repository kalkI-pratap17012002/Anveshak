...
class ContainerManager:
    ...
    def __init__(self, recon_image: str = "anveshak-recon:latest", gvm_image: str = "anveshak-vuln-scanner:latest", github_hunt_image: str = "anveshak-github-hunter:latest", trufflehog_image: str = "anveshak-trufflehog:latest"):
    ...
    def _get_container_name(self, project_id: str) -> str:
        ...
        return f"anveshak-recon-{safe_id}"
        ...
        try:
            ...
            container = self.client.containers.run(
                ...
                volumes={
                    ...
                    "/tmp/anveshak": {"bind": "/tmp/anveshak", "mode": "rw"},
                    # JS Recon shared volumes with webapp
                    "anveshak_js_recon_uploads": {"bind": "/data/js-recon-uploads", "mode": "ro"},
                    "anveshak_js_recon_custom": {"bind": "/data/js-recon-custom", "mode": "ro"},
    ...
    def _get_partial_container_name(self, project_id: str, run_id: str) -> str:
        ...
        safe_id = re.sub(r'[^a-zA-Z0-9_.-]', '_', project_id)
        return f"anveshak-partial-recon-{safe_id}-{run_id[:8]}"
    ...
    async def start_partial_recon(
        ...
        try:
            ...
            # Write config JSON to /tmp/anveshak/ (shared volume)
            import json
            config_dir = Path("/tmp/anveshak")
            ...
            container = self.client.containers.run(
                ...
                environment={
                    ...
                    "PARTIAL_RECON_CONFIG": f"/tmp/anveshak/partial_{project_id}_{run_id}.json",
    ...
    async def stop_partial_recon(self, project_id: str, run_id: str, timeout: int = 10) -> PartialReconState:
        ...
        try:
            config_path = Path(f"/tmp/anveshak/partial_{project_id}_{run_id}.json")
        ...
        try:
            ...
            container = self.client.containers.run(
                ...
                volumes={
                    ...
                    "anveshak_gvmd_socket": {"bind": "/run/gvmd", "mode": "ro"},
    ...
    def is_gvm_available(self) -> bool:
        ...
        try:
            container = self.client.containers.get("anveshak-gvm-gvmd")
    ...
    def _get_github_hunt_container_name(self, project_id: str) -> str:
        ...
        safe_id = re.sub(r'[^a-zA-Z0-9_.-]', '_', project_id)
        return f"anveshak-github-hunt-{safe_id}"
    ...
    def _get_trufflehog_container_name(self, project_id: str) -> str:
        ...
        safe_id = re.sub(r'[^a-zA-Z0-9_.-]', '_', project_id)
        return f"anveshak-trufflehog-{safe_id}"
