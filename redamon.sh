#!/usr/bin/env bash
# =============================================================================
# Anveshak CLI - Simplified installation, update, and lifecycle management
# =============================================================================
set -euo pipefail
...
TOOL_IMAGES="anveshak-recon:latest anveshak-vuln-scanner:latest anveshak-github-hunter:latest anveshak-trufflehog:latest anveshak-baddns:latest"
...
# One-time migration from the legacy `.skipkbase` flag (Anveshak <=4.9.3) to the
...
remove_anveshak_images() {
    # Remove locally-built anveshak images
...
pull_gvm_images() {
    ...
    if [[ ${#failed[@]} -gt 0 ]]; then
        ...
        echo -e "  ${YELLOW}Then: sudo systemctl restart docker && ./anveshak.sh up${NC}"
        exit 1
    fi
...
_kb_wait_neo4j() {
    if ! docker ps --format '{{.Names}}' | grep -q '^anveshak-neo4j$'; then
    ...
    while [[ $waited -lt $max_wait ]]; do
        ...
        health=$(docker inspect --format='{{.State.Health.Status}}' \
                   anveshak-neo4j 2>/dev/null || echo "unknown")
    ...
    error "Check: docker logs anveshak-neo4j"
    return 1
}

# Check if the agent container has a CUDA-capable GPU available.
_kb_has_gpu() {
    docker exec anveshak-agent python -c \
...
# should always succeed (or return a safe fallback) so `./anveshak.sh status`
# works in any state.
...
_kb_get_neo4j_count() {
    if ! docker ps --format '{{.Names}}' 2>/dev/null | grep -q '^anveshak-neo4j$'; then
        echo "0"
        return
    fi
    ...
    local count
    count=$(docker exec anveshak-neo4j cypher-shell \
...
cmd_install() {
    ...
    local version
    version="$(get_version)"
    info "Installing Anveshak v${version}..."
    ...
    echo -e "  ${GREEN}${BOLD}  Anveshak v${version} is ready!${NC}"
    ...
    if is_kb_enabled; then
        ...
        if _kb_bootstrap "$kb_profile"; then
            ...
            warn "Retry with: ./anveshak.sh kb build ${kb_profile}"
        fi
    else
    ...
    echo ""
    echo -e "  ${CYAN}Status:${NC}  ./anveshak.sh status"
    echo ""
    echo -e "  ${YELLOW}If Anveshak is useful to you, a GitHub star helps others find the project:${NC}"
    echo -e "  ${CYAN}https://github.com/samugit83/anveshak${NC}"
    echo ""
    if [[ "$gvm_mode" == "true" ]]; then
...
cmd_update() {
    ...
    if ! git -C "$SCRIPT_DIR" pull --ff-only 2>/dev/null; then
        if ! git -C "$SCRIPT_DIR" pull --ff-only origin master 2>/dev/null; then
            ...
            echo ""
            echo "  Try one of:"
            echo "    git stash && ./anveshak.sh update && git stash pop"
            echo "    git commit -am 'local changes' && ./anveshak.sh update"
            exit 1
        fi
    fi
...
cmd_up_dev() {
    ...
    info "Starting Anveshak in DEV mode (GVM: ${gvm_flag})..."
    ...
    echo ""
    echo -e "  ${GREEN}${BOLD}==========================================================${NC}"
    echo -e "  ${GREEN}${BOLD}  Anveshak DEV is ready!${NC}"
...
cmd_up() {
    ...
    info "Starting Anveshak (GVM: ${gvm_mode})..."
    ...
    echo ""
    echo -e "  ${GREEN}${BOLD}==========================================================${NC}"
    echo -e "  ${GREEN}${BOLD}  Anveshak is ready!${NC}"
    echo -e "  ${GREEN}${BOLD}  Open ${CYAN}http://localhost:3000${GREEN}${BOLD} in your browser${NC}"
    echo -e "  ${GREEN}${BOLD}==========================================================${NC}"
    echo ""
...
cmd_down() {
    info "Stopping Anveshak..."
    docker compose down
    success "All services stopped. Volumes and images preserved."
}

cmd_clean() {
    warn "This will remove all Anveshak containers and images."
    ...
    info "Removing Anveshak images..."
    remove_anveshak_images
    docker image prune -f >/dev/null 2>&1 || true

    success "All Anveshak containers and images removed. Volumes preserved."
    echo ""
    info "To reinstall: ./anveshak.sh install"
}

cmd_purge() {
    echo ""
    warn "This will PERMANENTLY DELETE:"
    warn "  - All Anveshak containers and images"
    ...
    rm -f "$KBASE_DISABLED_FLAG_FILE"
    rm -f "$LEGACY_SKIPKBASE_FLAG_FILE"
    success "Full cleanup complete. All Anveshak data and images have been removed."
    echo ""
    info "To reinstall: ./anveshak.sh install"
}
...
cmd_status() {
    ...
    # Container list — filter to anveshak containers only. Keeps the header
    # row and any container whose name starts with "anveshak-".
    docker compose ps | grep -E '^(NAME|anveshak-)' || {
...
cmd_kb_build() {
    ...
    case "$profile" in
        ...
        *)
            ...
            echo "Usage: ./anveshak.sh kb build [lite|standard|full]"
            exit 1
            ;;
    esac
...
cmd_kb_rebuild() {
    ...
    case "$profile" in
        ...
        *)
            ...
            echo "Usage: ./anveshak.sh kb rebuild [cpu-lite|lite|standard|full]"
            exit 1
            ;;
    esac
...
cmd_kb_help() {
    echo -e "${BOLD}Usage:${NC} ./anveshak.sh kb <command> [args]"
    echo ""
    ...
    echo "  full      standard + Nuclei (requires anveshak-kali container running)"
    echo ""
    echo -e "${BOLD}Examples:${NC}"
    echo "  ./anveshak.sh kb build             # Build lite KB (default)"
    echo "  ./anveshak.sh kb build standard    # Build with 2 years of NVD"
    echo "  ./anveshak.sh kb rebuild           # Wipe + rebuild standard (default)"
    echo "  ./anveshak.sh kb rebuild lite      # Wipe + rebuild lite profile"
    echo "  ./anveshak.sh kb rebuild full      # Wipe + rebuild full profile (incl. nuclei)"
    echo "  ./anveshak.sh kb update nvd        # Incremental NVD refresh"
    echo "  ./anveshak.sh kb update            # Update all sources"
    echo "  ./anveshak.sh kb stats             # See what's in the KB"
    echo ""
}

cmd_help() {
    print_banner
    echo -e "${BOLD}Usage:${NC} ./anveshak.sh <command> [options]"
    echo ""
    echo -e "${BOLD}Commands:${NC}"
    echo -e "  ${GREEN}install${NC}              Build and start Anveshak (no GVM, no Knowledge Base)"
    echo -e "  ${GREEN}install --gvm${NC}        Build and start Anveshak (with GVM/OpenVAS)"
    ...
    echo ""
    echo -e "${BOLD}Examples:${NC}"
    echo "  ./anveshak.sh install               # First-time setup (lightweight: no GVM, no KB)"
    echo "  ./anveshak.sh install --kbase       # First-time setup with local Knowledge Base"
    echo "  ./anveshak.sh install --gvm         # First-time setup with GVM/OpenVAS"
    echo "  ./anveshak.sh install --gvm --kbase # First-time setup with everything"
    echo "  ./anveshak.sh update           # Update to latest version"
    echo "  ./anveshak.sh up               # Start after reboot"
    echo "  ./anveshak.sh up dev           # Dev mode with hot-reload (auto-detects GVM)"
    echo "  ./anveshak.sh reset-password   # Reset a user's password"
    echo "  ./anveshak.sh kb build lite    # Build Knowledge Base"
    echo "  ./anveshak.sh kb update        # Refresh all KB sources"
    echo "  ./anveshak.sh kb stats         # Show KB chunk counts"
    echo ""
}
