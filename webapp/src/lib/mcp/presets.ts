 ...
 * MCP servers chosen for Anveshak's pentest workflow.
 *
...
export const MCP_PRESETS: McpPreset[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Research vulnerable libraries, understand exploit code in public PoC repos, audit dependencies before targeting them.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Find exploit PoCs, search for credentials in public repos, identify vulnerable code patterns, supply-chain analysis.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Find security research papers (e.g., recent CVE writeups, exploitation techniques), datasets for fuzzing, ML-based vuln-detection tools.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Look up current API surface of libraries you\'re probing (e.g., to understand authentication flows or find deprecated/removed protections).',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Independent OSINT search engine — useful when Tavily/Google rate limits hit during recon, or for cross-checking results from a different index.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Scrape target pages, fetch CVE advisories, pull from blog posts/changelogs/release notes — agent-side equivalent of curl with content extraction.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Carry findings across sessions: track confirmed credentials, persistent shells, mapped subdomain → CVE relationships beyond a single chat.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Force the LLM to plan exploit chains explicitly: enumerate steps, revise hypothesis, document decision points. Audit-trail-friendly.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Stash exfiltrated files, build wordlists/payloads incrementally, save reverse-shell artifacts between iterations. Sandboxed to /app/logs/sandbox.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Correlate event timestamps across logs, scheduling, time-zone-aware credential expiry checks. Best first preset to verify the integration works end-to-end.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Map external attack surface, find exposed services, banner-grab without touching the target. Augments execute_naabu by adding pre-existing scan data + CVE matches.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Triage suspicious artifacts found during scans, check IP reputation before exfil callbacks, identify known-malicious domains hosted on target subdomains.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Single-call DNS recon (lookups, reverse, SPF chain, DMARC, wildcards, SRV), CT log search (crt.sh), passive ASN/BGP analysis. Fills gaps the kali-sandbox tools don\'t cover natively.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'IP reputation enrichment — distinguish opportunistic scanners from targeted attackers, validate exfil destinations, check WAF blocklists. Multiple feeds cross-corroborate.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'When you exfil source or have access to a target\'s public GitHub: scan it for SQLi, XSS, SSRF, hard-coded secrets, deserialization gadgets. Custom-rule support for one-off pattern hunts.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Backup for built-in web_search if Tavily key isn\'t set globally; also exposes extract/crawl tools the built-in lacks. Useful when scoping a target — fetch their entire site.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Find writeups by description ("recent CVE-2024 RCEs in Java deserializers"), discover similar PoCs to a known exploit, semantic search for vendor-specific advisories.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Stealth-mode OSINT search that doesn\'t correlate with your project\'s Tavily/Brave keys. Useful when you want zero attribution to your infra during attribution-sensitive recon.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Once SQLi grants DB access, point this at the captured DSN to enumerate schemas/tables/users/permissions structurally — much cleaner than crafting SQL through the injection point.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Independent browser channel: run a parallel exploit with Puppeteer while system execute_playwright handles the primary session. Useful for SAML/OAuth flows where two browser contexts are needed.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Notify your team Slack when the agent confirms a vuln, completes a phase, or needs human approval. Also useful as a low-noise output channel during long unattended scans.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Background on a target organization, vendor history, biographical info on email enumeration targets. Neutral source independent of your search-API rate limits.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Drives ZAP\'s active scanner from the agent — automated XSS/SQLi/SSRF/path-traversal hunts on web targets, with payload variation that nuclei templates don\'t cover.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'When you obtain leaked AWS credentials (e.g., via execute_gau finding env files, GitHub PAT exfil), enumerate S3 buckets, IAM users/roles, DynamoDB tables, and identify privilege-escalation paths.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Independent corroboration of Shodan data; Censys often catches assets Shodan misses. Strong on TLS/cert intel — find shadow IT subdomains via SAN-list mining.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Phishing simulation prep, social-engineering target enumeration, AD username-pattern discovery (often emails == usernames). Free tier 25 req/month.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Check whether emails captured during recon appear in known breaches → predict valid passwords for credential stuffing. Domain-wide search reveals breach exposure for the entire target.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Active web-app pentest beyond what curl can do — capture mobile-app / SPA traffic, manipulate JWT tokens mid-flight, replay session-fixation payloads. Complements execute_playwright.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Triage exfiltrated SQLite files (mobile-app DBs, Chrome cookies/history, Slack desktop, Signal, password managers). Many high-value artifacts ship as .db files.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Stream findings into a structured engagement workspace as the agent confirms vulns. Pages can become draft sections of the final pentest report. 22 tools.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'When the local Playwright/Puppeteer hits aggressive bot detection (Cloudflare Turnstile, PerimeterX, Akamai), Browserbase provides residential-IP-backed sessions with anti-detection profiles.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'When a target uses Kubernetes and you obtain a kubeconfig (leaked, RBAC misconfig, privileged pod escape), enumerate namespaces, secrets, service accounts, and look for privilege-escalation paths.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Scan target source you\'ve exfiltrated or have access to (open-source dependencies, package.json, requirements.txt, etc.) for known CVEs and license risks.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'When pentesting an e-commerce target that uses Stripe, you can validate the merchant\'s test-mode flow, simulate fraudulent card patterns, and check webhook signature verification with safe test keys.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Convert each confirmed vulnerability into a Linear issue automatically (severity, repro steps, evidence). Sync with the client\'s remediation team during long engagements.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Scan target Docker images / Kubernetes manifests / Terraform after exfil — identifies vulnerable base images, hard-coded secrets, IaC misconfig (open security groups, public S3, etc).',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'When you obtain cloud credentials, runs 300+ checks against the target tenancy: IAM mistakes, public S3, weak RDS encryption, open NSGs, etc. Catches things the basic AWS MCP can\'t.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'Deeper CVE context than the built-in cve_intel tool: EPSS exploitation probability, CISA known-exploited status, ATT&CK technique mappings, OSV cross-reference. Useful for prioritizing exploit targets.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForAnveshak: 'When you exfiltrate proprietary binaries (Linux ELF, Windows PE, mobile-app native libs), this lets the LLM disassemble / decompile / search strings + xrefs to find auth bypasses, hard-coded secrets, vulns.',
