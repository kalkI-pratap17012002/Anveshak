...
- **BadDNS (AGPL-3.0 isolated sidecar)** with 7 modules:
  ...
  - Runs in its own Docker image (\`anveshak-baddns:latest\`), zero license contagion.
...
- **\`subjackCheckAr\`** (stale A record detection) probes cloud IP ranges and historically needed root/ICMP. Anveshak's recon container runs with the right privileges so this is fine, but expect some false positives -- those are exactly the findings the manual-review queue is for.
...
- **Manual-review noise**: auto-publish is ON, so stale-A and low-confidence findings appear as \`severity: medium\`. Filter by \`verdict\` in the Findings table to triage -- \`confirmed\` first, then \`likely\`, then \`manual_review\`.`,
  parameters: {
    ...
    // Runs in its own Docker image (anveshak-baddns:latest); no license contagion
    // since Anveshak never imports baddns and communicates over stdout only.
